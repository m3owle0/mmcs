...
serve(async (req) => {
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      console.log(`Processing notifications subscription for: ${customerEmail}`)

      // Only handle notifications subscription ($10.00 = 1000 cents)
      if (session.amount_total !== 1000) {
        console.log(`Ignoring non-notifications subscription (amount: ${session.amount_total} cents)`)
      ...
      // Update user in Supabase - activate notifications subscription
      const { data, error } = await supabaseAdmin
        .from('unlocked_users')
        .update({
          notifications_subscription_active: true,
          notifications_subscription_expires_at: expiresAt.toISOString(),
      ...
      console.log(`✅ Successfully activated notifications subscription for ${customerEmail}`)
    ...
    if (event.type === 'customer.subscription.updated') {
      ...
      // Update notifications subscription expiry
      ...
      const { error: updateError } = await supabaseAdmin
        .from('unlocked_users')
        .update({
          notifications_subscription_active: isActive,
          notifications_subscription_expires_at: expiresAt.toISOString()
        })
        .eq('email', user.email)
    ...
    if (event.type === 'customer.subscription.deleted') {
      ...
      if (user) {
        await supabaseAdmin
          .from('unlocked_users')
          .update({
            notifications_subscription_active: false,
